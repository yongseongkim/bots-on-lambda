...
(async () => {
  try {
    ...
    console.log('법원경매정보 완료')

    const indexFrame = page.frame('indexFrame')
    const searchButton = await indexFrame.locator('div#main_btn');
    await searchButton.click()

    const listItemsFrame = page.frame('indexFrame')
    await listItemsFrame.waitForSelector('table.Ltbl_list');
    console.log('물건상세검색 완료')

    const table = await listItemsFrame.locator('table.Ltbl_list')
